...
def open_site_manager_hub(
    ...
    header = tk.Frame(window, bg=COLORS["header_bg"], height=70)
    ...
    def _render_overview() -> None:
        ...
        left_panel = _create_card(content_frame)
        ...
        right_panel = tk.Frame(content_frame, bg=COLORS["bg_main"])
        right_panel.grid(row=0, column=1, sticky="nsew", padx=(12, 0))
        right_panel.grid_rowconfigure(1, weight=1)
        
        # Alerts card
        alerts_card = _create_card(right_panel)
        alerts_card.pack(fill="x", padx=0, pady=(0, 12))
        ...
        alert_items = [
            (f"Missing Reports", f"{metrics_data.get('missing_reports', 0)} reports", COLORS["accent_red"], "Today"),
            ("Delayed Projects", f"{metrics_data.get('delayed_projects', 0)} projects", COLORS["accent_orange"], "Today"),
            ("Tasks Overdue", "3 tasks", COLORS["warning"], "2 days ago"),
            ("Equipment Issues", f"{metrics_data.get('equipment_issues', 0)} issues", COLORS["accent_blue"], "Today"),
            ("Low Stock", f"{len(metrics_data.get('low_stock_alerts', []))} items", COLORS["accent_green"], "1 hour ago"),
        ]
        
        for title, value, color, time in alert_items:
            alert_item = tk.Frame(alerts_card, bg=COLORS["card_bg"])
            alert_item.pack(fill="x", padx=12, pady=4)
            
            info_frame = tk.Frame(alert_item, bg=COLORS["card_bg"])
        ...
        tk.Button(
            ...
            command=lambda: _show_toast(window, "Opening Alerts"),
            bg=COLORS["card_bg"],
            fg=COLORS["accent_orange"],
        ...
        # Project Progress
        projects_card = _create_card(right_panel)
        projects_card.pack(fill="x", padx=0, pady=(0, 12))
        ...
        projects = list_projects()[:3]
        for project in projects:
            proj_item = tk.Frame(projects_card, bg=COLORS["card_bg"])
            proj_item.pack(fill="x", padx=12, pady=4)
            ...
            progress_color = COLORS["accent_green"] if progress >= 75 else COLORS["accent_orange"] if progress >= 50 else COLORS["accent_red"]
            
            bar_frame = tk.Frame(proj_item, bg=COLORS["bg_main"], height=4)
            bar_frame.pack(fill="x", pady=4)
            
            filled = tk.Frame(bar_frame, bg=progress_color, height=4)
            filled.place(relwidth=progress/100, relheight=1)
            
            tk.Label(
                proj_item,
                text=f"{progress}% • {project.get('status', 'Unknown')}",
                fg=COLORS["text_muted"],
                bg=COLORS["card_bg"],
                font=FONTS["small"],
            ).pack(anchor="w")
        ...
        calendar_card = _create_card(right_panel)
        calendar_card.pack(fill="both", expand=True, padx=0)
