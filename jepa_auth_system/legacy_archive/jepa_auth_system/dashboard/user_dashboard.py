...
def user_dashboard(user_id: int, username: str, saved_theme: str = "dark"):
        ...
        with get_connection() as conn:
            row = conn.execute(
                """SELECT profile_pic, full_name, country_code, phone,
                          gender, dob, bio, theme, email, totp_secret
                   FROM users WHERE id=?""",
                (user_id,),
            ).fetchone()
        if row:
    ...
    def _apply_theme():
        ...
        hdr_lbl.config(bg=t["bg"], fg=t["fg"])
    ...
    # ── Header ─────────────────────────────────────────────────────────────────
    hdr_frame = tk.Frame(dash, bg="#0F172A")
    hdr_frame.pack(fill="x", padx=30, pady=(20, 10))
    hdr_lbl = tk.Label(hdr_frame, text="ACCOUNT PROFILE", font=FONTS["brand"])
    ...
    # ── Profile Card ───────────────────────────────────────────────────────────
    card_frame = tk.Frame(dash, highlightthickness=1, bd=0)
    ...
    def _section(title):
        tk.Label(card_frame, text=title, font=FONTS["label"], fg="#0EA5E9", bg=card_frame["bg"]).pack(pady=(10, 2))
        lbl = tk.Label(card_frame, font=FONTS["body"])
        lbl.pack(pady=(0, 10))
        return lbl

    lbl_phone = _section("CONTACT DETAILS")
    lbl_demo  = _section("DEMOGRAPHICS")
    lbl_bio   = _section("PROFILE SUMMARY")
    ...
    tk.Button(dash, text="✏️  Edit Profile", command=_edit_profile, bg="#0EA5E9", fg="white", **btn_cfg).pack(pady=(14, 4), padx=30, fill="x", ipady=4)
    tk.Button(dash, text="🔑  Change Password", command=_change_password, bg="#334155", fg="#F8FAFC", **btn_cfg).pack(pady=4, padx=30, fill="x", ipady=4)
    ...
    tk.Button(dash, text="🛡️  Setup Two-Factor (2FA)", command=_setup_2fa, bg="#10B981", fg="white", **btn_cfg).pack(pady=4, padx=30, fill="x", ipady=4)
